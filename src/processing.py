...
def sort_by_date(input_list: list[dict], sorting = False) -> list:
    """функция сортирует принимаемы список по дате
    :param input_list:  список словарей
    :param sorting: необязательный параметр, задающий порядок сортировки (по умолчанию — убывание)
    """
    return sorted(input_list, key = lambda n_dict : n_dict['date'], reverse = not sorting)
